...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_email_phone_number(self, value):
        ...
        if value and User.objects.filter(email=value).exists():
            data = {
                "success": False,
                "message": "Email allaqachin mavjud",
            }
            raise ValidationError(data)
        elif value and User.objects.filter(phone_number=value).exists():
            data = {
                "success": False,
                "message": "Telefon raqam allaqachin mavjud",
            }
            raise ValidationError(data)
        return value
...
class ChangeUserInformatoion(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if password != confirm_password:
            raise ValidationError(
                {
                    'messages':'Parolingiz bir-biriga teng emas'
                }
            )
        if password:
    ...
    def validate_username(self, username):
        ...
        if username.isdigit():
            raise ValidationError(
                {
                    "message":"This username can not be a number"
                }
            )
        return username
...
class LoginSerializer(TokenObtainPairSerializer):
    ...
    #         username = user.username if user else None
    #     else:
    #         raise ValidationError({"success": False, "message": "Invalid input format"})
    #
    #     if not username:
    ...
    #         self.user = user
    #     else:
    #         raise ValidationError({"success": False, "message": "Invalid login credentials"})
    ...
    def auth_validate(self, data):
        ...
        if check_user_type(user_input) == 'username':
            ...
            user = self.get_user(
                email__iexact=user_input)  # user get method orqali user o'zgartiruvchiga biriktirildi
    ...
    def validate(self, data):
        ...
        if self.user.auth_status not in [DONE, PHOTO_DONE]:
            raise PermissionDenied("You can't login now")
    ...
    def validate(self, attrs):
        ...
        if email_or_phone is None:
            raise ValidationError(
                {
                    "success":False,
                    'message':"Email yoki telefon raqam kiritilishi kerak."
                }
            )
